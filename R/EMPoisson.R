                                  ...
                                  penalty, hyper.params)
{
  ...
  # Take old parameters
  mean.theta = poisson.params.old[1]
  # Take hyper parameters
  hyper.mean.1 = hyper.params[1]
  hyper.mean.2 = hyper.params[2]
  # Value to return
  poisson.params.new = poisson.params.old
  ...
  sum.one = sweep(matrix(z.e.obs[pos.idx]), 1, sweep(matrix(z.e.lat[pos.idx]), 1, matrix(k.e[pos.idx]), FUN = "*", check.margin = FALSE), FUN = "+", check.margin = FALSE)
    # z.e.obs[pos.idx] + k.e[pos.idx] * z.e.lat[pos.idx]
  sum.two = sweep(sweep(matrix(z.e.obs[pos.idx]), 1, matrix(y.e.obs[pos.idx]), FUN = "*", check.margin = FALSE), 1,
  ...
  mean.theta.new = (apply(sum.two, 2, FUN = "sum") - (hyper.mean.1-1)) / (apply(sum.one, 2, FUN = "sum") + 1/hyper.mean.2)
    # (sum(sum.two) - (hyper.mean.1-1)) / (sum(sum.one) + 1/hyper.mean.2)

  poisson.params.new[1] = mean.theta.new

  return(poisson.params.new)
