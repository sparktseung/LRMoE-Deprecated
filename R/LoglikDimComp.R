...
#'
#' @param Y An N*4d matrix of numerics, where N is the sample size and d is the dimension of observation.
#'          For each dimension, the four columns shoud be organized as \code{(tl, yl, yu, tu)}.
#' @param comp.dist A d*g matrix of strings, where d is the dimension of observation and g is the number of components.
#' @param zero.init A d*g matrix of numerics between 0 and 1, representing zero inflation by dimension and by component.
...
#' @return \code{pos.expert.ll, pos.expert.tn, pos.expert.tn.bar, expert.ll, expert.tn, expert.tn.bar} See \code{\link{pos.expert.loglik.calc}}.
...
expert.loglik.dim.comp = function(Y, comp.dist, zero.init, params.init)
{
  ...
  for(k in 1:dim.m) # Loop through dim(y), i.e. the rows
  {
    ...
    for(j in 1:n.comp)
    {
      ...
      yu = Y[,(4*(k-1)+3)]
      tu = Y[,(4*(k-1)+4)]
      temp.dim.pos.expert.list = pos.expert.loglik.calc(comp.dist[k,j],
      ...
      obs.y.zero.idx = (yl==0) # yl=0: possible for observation to be from zero component
      obs.tn.zero.idx = (tl==0) # tl=0: possible for the truncation interval to include zero
      obs.exact.zero.idx = (tu==0) # tu=0: the ovservation is exactly zero, subset of preceding two cases
