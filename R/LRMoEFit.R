...
#' @param hyper.alpha A g*P matrix of numerics, which contains penalties for \code{alpha.init}.
...
#' @export LRMoE.fit
#'
LRMoE.fit = function(Y, X, n.comp = 2, comp.dist = NULL,
                     alpha.init = NULL,
                     ...
                     print = TRUE)
{
  ...
  if(is.null(hyper.params)){
    ...
    for(k in 1:dim.m){ for(j in 1:n.comp) { hyper.params[[k]][[j]] = hyper.params.kj.initialize(comp.dist[k,j]) }  }
  }
  ...
  gate.ll.init = gate.logit(X, alpha.init) # Dimension is: sample.size.n * n.comp

  # Initialize Expert functions
  expert.list.init = expert.loglik.dim.comp(Y, comp.dist, zero.init, params.init)

  # Calculate various parts of the loglik. It is a LONG and detailed list of numbers and matrices
  ll.list.init = gate.expert.loglik(alpha.init, gate.ll.init, expert.list.init, penalty, hyper.alpha, hyper.params)
  ...
  zero.em = array(0, dim = c(dim.m, n.comp)) # This copy goes into EM
  ...
  expert.list.em = expert.loglik.dim.comp(Y, comp.dist, zero.em, params.em) # This copy goes into EM
  ll.list.em = gate.expert.loglik(alpha.em, gate.ll.em, expert.list.em, penalty, hyper.alpha, hyper.params) # This copy goes into EM
  ...
  while( (loglik.em - loglik.em.old > eps) & iter<ecm.iter.max )
  {
    ...
    comp.zkz.e.list = comp.zkz.e.recur(gate.ll.em, expert.list.em, ll.list.em)

    # M-Step: gating weights, i.e. alpha
    alpha.old = alpha.em
    # hyper.alpha = 5 ###### Only for testing purposes #####
    alpha.em = alpha.m.recur(X, alpha.old, comp.zkz.e.list, alpha.iter.max, penalty, hyper.alpha)

    # Check that loglik increases after M-step of alpha
    gate.ll.em = gate.logit(X, alpha.em)
    expert.list.em = expert.loglik.dim.comp(Y, comp.dist, zero.em, params.em)
    ll.list.em = gate.expert.loglik(alpha.em, gate.ll.em, expert.list.em, penalty, hyper.alpha, hyper.params) # This copy goes into EM
    loglik.em = ll.list.em$ll
    ...
    for(k in 1:dim.m)
    {
      for(j in 1:n.comp)
      {
        ...
        # comp.kj.zero.prob.old   = expert.list[[k]]$zero.prob[j]
        # comp.kj.params.old      = expert.list[[k]]$pos.params[[j]]
        ...
        z.zero.e.obs = comp.zkz.e.list$z.e.obs[,j] * z.zero.e.obs.recur(yl.k, comp.kj.zero.inflation, comp.kj.zero.prob.old, comp.kj.pos.expert.ll)
        ...
        if(comp.kj.zero.inflation==TRUE) # Only do this for zero-inflated distributions
        {
          ...
          expert.list.em = expert.loglik.dim.comp(Y, comp.dist, zero.em, params.em)
          ll.list.em = gate.expert.loglik(alpha.em, gate.ll.em, expert.list.em, penalty, hyper.alpha, hyper.params)
          ...
          gate.ll.em = gate.logit(X, alpha.em) # alpha.em has been updated
          expert.list.em = expert.loglik.dim.comp(Y, comp.dist, zero.em, params.em) # zero.em[k,j] has been updated
          ll.list.em = gate.expert.loglik(alpha.em, gate.ll.em, expert.list.em, penalty, hyper.alpha, hyper.params)
        ...
        gate.ll.em = gate.logit(X, alpha.em) # alpha.em has been updated
        expert.list.em = expert.loglik.dim.comp(Y, comp.dist, zero.em, params.em) # zero.em has been updated
        ll.list.em = gate.expert.loglik(alpha.em, gate.ll.em, expert.list.em, penalty, hyper.alpha, hyper.params)
        loglik.em.before.kj = ll.list.em$ll
        ...
        params.em[[k]][[j]] = comp.kj.params.m.recur(comp.kj.dist, comp.kj.params.old,
                                                     ...
                                                     z.pos.e.obs, z.pos.e.lat, comp.zkz.e.list$k.e, # comp.zkz.e.list$k.e[,k], # comp.zkz.e.list$k.e,
        ...
        no.jump.list = c("nbinom", "ZI-nbinom")
        ...
        if( grad.jump==TRUE & (iter%%grad.period)==0 & !(comp.kj.dist %in% no.jump.list) ){
          ...
          for(t in 1:grad.length)
          {
            ...
            gate.ll.em = gate.logit(X, alpha.em)
            expert.list.em = expert.loglik.dim.comp(Y, comp.dist, zero.em, params.temp) # params.em[[k]][[j]] has been updated with gradient
            ll.list.em.temp = gate.expert.loglik(alpha.em, gate.ll.em, expert.list.em, penalty, hyper.alpha, hyper.params)
        ...
        if(loglik.em.after.kj>0 | is.infinite(loglik.em.after.kj)){ # No update, if the loglik becomes Inf
          if(print){
            print(paste("No update on component: Dim ", k, " Comp ", j, " !" ));
  ...
  for(k in 1:dim.m){
    ...
    for(j in 1:n.comp){
      ...
      names(params.init[[k]][[j]]) = names(params.em[[k]][[j]]) = name.y.pos(comp.dist[k,j])
    }
  }
