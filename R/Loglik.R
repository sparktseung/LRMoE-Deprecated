...
#'
#' @param alpha A g*P matrix, where g is the number of components and P is the number of covariates.
#' @param gate.ll An object returned by function \code{\link{gate.logit}}.
#' @param expert.list An object returned by function \code{\link{expert.loglik.dim.comp}}.
#' @param penalty TRUE/FALSE, which indicates whether parameter penalty should be applied. Default (and recommended) is TRUE.
#' @param hyper.alpha A g*P matrix, which penalizes the magnitude of \code{alpha}.
...
gate.expert.loglik = function(alpha, gate.ll, expert.list, penalty = TRUE, hyper.alpha = NULL, hyper.params = NULL)
{
  ...
  if(penalty == TRUE)
  {
    ...
    for(k in 1:dim.m)
    {
      for(j in 1:n.comp)
      {
        penalty.mat[k,j] = expert.loglik.pen.dim.comp(expert.list[[k]]$dim.comp.dist[j], expert.list[[k]]$pos.params[[j]], hyper.params[[k]][[j]])
      }
    }
