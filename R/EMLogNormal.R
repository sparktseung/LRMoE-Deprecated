                                ...
                                penalty, hyper.params)
{
  ...
  term.zkz = sweep(matrix(z.e.obs[pos.idx]), 1,
  ...
  meanlog.new = apply(term.zkz.logy, 2, FUN = "sum") / apply(term.zkz, 2, FUN = "sum")
    # sum(term.zkz.logy) / sum(term.zkz)
  sdlog.new = sqrt( 1/apply(term.zkz, 2, FUN = "sum") * ( apply(term.zkz.logy.sq, 2, FUN = "sum") - 2*meanlog.new*apply(term.zkz.logy, 2, FUN = "sum") + (meanlog.new^2)*apply(term.zkz, 2, FUN = "sum") ) )
