                                 ...
                                 penalty, hyper.params)
{
  ...
  # Take old parameters
  size.n = nbinom.params.old[1]
  prob.p = nbinom.params.old[2]
  # Take hyper parameters
  hyper.n.1 = hyper.params[1]
  hyper.n.2 = hyper.params[2]
  # Value to return
  nbinom.params.new = nbinom.params.old
  ...
  y.e.lat.unique[,1] = size.n*(1-prob.p)/prob.p - #size.n*prob.p/(1-prob.p) -
    mapply(function(x, y) ifelse(x!=y,
                                 int.y.fcn(x, y, size.n, prob.p),
                                 0),
           tl.unique, tu.unique)
  ...
  prob.size = function(size.n, z.e.obs, z.e.lat, k.e, y.e.obs, y.e.lat)
  {
    ...
    temp.p = 1 - apply(term.zkz.y, 2, FUN = "sum") / ( size.n*apply(term.zkz, 2, FUN = "sum") + apply(term.zkz.y, 2, FUN = "sum") )
      # 1 - sum(term.zkz.y) / (size.n*sum(term.zkz) + sum(term.zkz.y))
    return(temp.p)
  }
                    ...
                    penalty, hyper.n.1, hyper.n.2)
  {
    ...
    result = apply(term.zkz.ylfac, 2, FUN = "sum") - apply(term.zkz, 2, FUN = "sum")*lfactorial(size.n.new-1) + apply(term.zkz, 2, FUN = "sum")*size.n.new*log(temp.p) + apply(term.zkz.y, 2, FUN = "sum")*log(1-temp.p)
      # sum(term.zkz.ylfac) - sum(term.zkz)*lfactorial(size.n.new-1) + sum(term.zkz)*size.n.new*log(temp.p) + sum(term.zkz.y)*log(1-temp.p)
    if(penalty==TRUE)
    {
  ...
  nbinom.params.new[1] = size.n.new
  nbinom.params.new[2] = prob.p.new

  return(nbinom.params.new)
