                                ...
                                penalty, hyper.params)
{
  ...
  # Take old parameters
  m = gamma.params.old[1]
  theta = gamma.params.old[2]
  ...
  # Value to return
  gamma.params.new = gamma.params.old
  ...
  theta.m = function(m, z.e.obs, z.e.lat, k.e, y.e.obs, y.e.lat, nu.m, lambda.m, nu.theta, lambda.theta)
  {
    ...
    quad.first = ( m*apply(term.zkz, 2, FUN = "sum") - (nu.theta-1) )^2
      # ( m*sum(term.zkz) - (nu.theta-1) )^2
    quad.second = (4/lambda.theta) * apply(term.zkz.y, 2, FUN = "sum")
      # (4/lambda.theta) * sum(term.zkz.y)

    theta = (lambda.theta/2) * ( (nu.theta-1) - m*apply(term.zkz, 2, FUN = "sum") + sqrt(quad.first + quad.second)  )
      # (lambda.theta/2) * ( (nu.theta-1) - m*sum(term.zkz) + sqrt(quad.first + quad.second)  )
  ...
  Q.m = function(m, z.e.obs, z.e.lat, k.e, y.e.obs, y.e.lat, y.log.e.obs, y.log.e.lat, nu.m, lambda.m, nu.theta, lambda.theta)
  {
    ...
    temp = (m-1)*apply(term.zkz.logy, 2, FUN = "sum") - (1/theta)*apply(term.zkz.y, 2, FUN = "sum") - (m*log(theta) + lgamma(m))*apply(term.zkz, 2, FUN = "sum") + ((nu.m-1)*log(m) - m/lambda.m) + ((nu.theta-1)*log(theta) - theta/lambda.theta)
      # (m-1)*sum(term.zkz.logy) - (1/theta)*sum(term.zkz.y) - (m*log(theta) + lgamma(m))*sum(term.zkz) + ((nu.m-1)*log(m) - m/lambda.m) + ((nu.theta-1)*log(theta) - theta/lambda.theta)
  ...
  gamma.params.new[1] = m.new
  gamma.params.new[2] = theta.new
  return(gamma.params.new)
