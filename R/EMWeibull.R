                                  ...
                                  penalty, hyper.params)
{
  ...
  # Take old parameters
  shape.k = weibull.params.old[1]
  scale.lambda = weibull.params.old[2]
  ...
  # Value to return
  weibull.params.new = weibull.params.old
  ...
  scale.shape = function(shape.k, z.e.obs, z.e.lat, k.e, y.pow.e.obs, y.pow.e.lat)
  {
    ...
    temp.scale = ( apply(term.zkz.ypow, 2, FUN = "sum")/apply(term.zkz, 2, FUN = "sum") )^(1/shape.k)
      # ( sum(term.zkz.ypow)/sum(term.zkz) )^(1/shape.k)
    return(temp.scale)
  }
                     ...
                     penalty, nu.shape, theta.shape)
  {
    ...
    result = log(shape.k.new)*apply(term.zkz, 2, FUN = "sum") - shape.k.new*log(scale.shape.new)*apply(term.zkz, 2, FUN = "sum") + (shape.k.new-1)*apply(term.zkz.ylog, 2, FUN = "sum") - scale.shape.new^(-shape.k.new)*apply(term.zkz.ypow, 2, FUN = "sum")
      # log(shape.k.new)*sum(term.zkz) - shape.k.new*log(scale.shape.new)*sum(term.zkz) + (shape.k.new-1)*sum(term.zkz.ylog) - scale.shape.new^(-shape.k.new)*sum(term.zkz.ypow)
  ...
  # Return results
  weibull.params.new[1] = shape.k.new
  weibull.params.new[2] = scale.lambda.new
  return(weibull.params.new)
