...
#' @keywords internal
#'
# #' @export binom.params.m.recur
binom.params.m.recur = function(binom.params.old,
                                tl, yl, yu, tu,
                                ...
                                penalty, hyper.params)
{
  ...
  # Take old parameters
  size.n = binom.params.old[1]
  prob.p = binom.params.old[2]
  # Take hyper parameters
  # hyper.mean.1 = hyper.params[1]
  # hyper.mean.2 = hyper.params[2]
  # Value to return
  binom.params.new = binom.params.old
  ...
  sum.one = z.e.obs[pos.idx] * y.e.obs[pos.idx] + k.e[pos.idx] * z.e.lat[pos.idx] * y.e.lat[pos.idx]
  sum.two = z.e.obs[pos.idx] * (size.n-y.e.obs[pos.idx]) + k.e[pos.idx] * z.e.lat[pos.idx] * (size.n-y.e.lat[pos.idx])
  prob.p.new = (sum(sum.one)) / (sum(sum.one) + sum(sum.two))

  binom.params.new[2] = prob.p.new

  return(binom.params.new)
