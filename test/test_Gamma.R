...
zero.prob = matrix(c(0, 0.3,
                     0.5, 0),
                   nrow = d, byrow = TRUE)
params.list = list(list(c(2, 1), c(1, 0.5)),
                   list(c(5, 2), c(2, 1)))
...
Y.gamma = dataset.simulator(X, alpha, comp.dist, zero.prob, params.list)
...
zero.guess = matrix(c(0, 0.8,
                      0.3, 0),
                    nrow = d, byrow = TRUE)
params.guess = list(list(c(1.5, 0.5), c(2, 0.4)),
                    list(c(4, 1), c(1.5, 1)))

Y.obs = cbind(rep(0, N), Y.gamma[,1], Y.gamma[,1], rep(Inf, N),
              rep(0, N), Y.gamma[,2], Y.gamma[,2], rep(Inf, N))
