...
zero.prob = matrix(c(0, 0.3,
                     0.5, 0),
                   nrow = d, byrow = TRUE)
params.list = list(list(c(5), c(10)),
                   list(c(6), c(8)))
...
Y.pois = dataset.simulator(X, alpha, comp.dist, zero.prob, params.list)
...
zero.guess = matrix(c(0, 0.8,
                      0.3, 0),
                    nrow = d, byrow = TRUE)
params.guess = list(list(c(5), c(10)),
                  list(c(8), c(8)))

Y.obs = cbind(rep(0, N), Y.pois[,1], Y.pois[,1], rep(Inf, N),
              rep(0, N), Y.pois[,2], Y.pois[,2], rep(Inf, N))
